                    ...
                    if (query.Any())
                    {
                        ...
                        foreach (var item in query)
                        {
                            if (item.DOUBLETAKE == "1")
                            {
                                ...
                                info += Convert.ToInt32(item.CIGZ).ToString().PadLeft(3, '0') + ","; //Convert.ToInt32(item.CIGZ).ToString().PadLeft(3, '0') + ","; //GlobalPara.BoxLenght/2 + ",";// (GlobalPara.BoxWidth / 2) + ",";//坐标Y
        ...
        public bool TaskLocate(decimal packagenum,decimal cigseq,decimal fbTaskNum,decimal yxyTaskNum)
        {
            try
            {
                using (Entities entity = new Entities())
                {
                    ...
                    {
                        //更新这个包号之前的任务为完成(不包括）
                        var query1 = (from item in entity.T_PACKAGE_TASK where item.PACKTASKNUM < packagenum   && item.CIGTYPE =="2"  && item.PACKAGENO == packageno && item.CIGSTATE != 20  select item).ToList();
                        if (query1.Any())
                        {
                    ...
                    {
                        var fbQuery1 = (from item in entity.T_PACKAGE_TASK where item.PACKTASKNUM < fbTaskNum  && item.PACKAGENO == packageno && item.NORMAILSTATE != 20 select item).ToList();
                        if (fbQuery1.Any())
                        {
                    ...
                    if( yxyTaskNum > 0) //异型烟倍速链
                    {
                        //倍速链
                        //合包标志更新成完成 小于这个任务号的）
                        var yxyQuery1 = (from item in entity.T_PACKAGE_TASK where item.PACKTASKNUM < yxyTaskNum   && item.PACKAGENO == packageno && item.STATE != 20 select item).ToList();
                        if (yxyQuery1.Any())
                        {
